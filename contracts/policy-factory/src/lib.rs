...
#[contract]
pub struct PolicyFactoryContract;

#[contractimpl]
impl PolicyFactoryContract {
...
mod test {
    ...
    fn test_initialize() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, PolicyFactoryContract);
        let client = PolicyFactoryContractClient::new(&env, &contract_id);
